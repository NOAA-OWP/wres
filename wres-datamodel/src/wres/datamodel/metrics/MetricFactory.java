...
/**
 * Factory class for creating {@link MetricConstants} from declaration.
 * 
 * @author James Brown
 */
...
public class MetricFactory
{
                                                             ...
                                                             ProjectConfig projectConfig )
    {
        ...
        returnMe.addAll( MetricFactory.getOrdinaryMetricsFromConfig( metricsConfig, projectConfig ) );

        returnMe.addAll( MetricFactory.getTimeSeriesMetricsFromConfig( metricsConfig, projectConfig ) );
                                                                       ...
                                                                       ProjectConfig projectConfig )
    {
        ...
        for ( TimeSeriesMetricConfig next : metricsConfig.getTimeSeriesMetric() )
        {
            ...
            if ( next.getName() == TimeSeriesMetricConfigName.ALL_VALID )
            {
                ...
                SampleDataGroup inGroup = MetricFactory.getMetricInputGroup( projectConfig.getInputs().getRight() );

                // Single-valued input source
                if ( inGroup == SampleDataGroup.SINGLE_VALUED )
                {
                    allValid = MetricFactory.getAllValidMetricsForSingleValuedTimeSeriesInput( projectConfig,
                                                                                               metricsConfig );
                }
                // Unrecognized type
                else
            ...
            {
                MetricConstants metricName = MetricFactory.getMetricName( next.getName() );
                returnMe.add( metricName );
                Set<MetricConstants> summaryStatistics = MetricFactory.getSummaryStatisticsFor( next );
                                                                     ...
                                                                     ProjectConfig projectConfig )
    {
        ...
        for ( MetricConfig next : metricsConfig.getMetric() )
        {
            ...
            if ( next.getName() == MetricConfigName.ALL_VALID )
            {
                ...
                {
                    allValid =
                            MetricFactory.getAllValidMetricsForSingleValuedInput( projectConfig, metricsConfig );
                }
                // Ensemble metrics
                else if ( inGroup == SampleDataGroup.ENSEMBLE )
                {
                    allValid = MetricFactory.getAllValidMetricsForEnsembleInput( projectConfig, metricsConfig );
                }
                // Unrecognized type
                else
                {
                                                                            ...
                                                                            MetricsConfig metricsConfig )
    {
        ...
        if ( !metricsConfig.getMetric().isEmpty() )
        {
            ...
            if ( MetricFactory.hasThresholds( metricsConfig, ThresholdType.PROBABILITY )
                 || MetricFactory.hasThresholds( metricsConfig, ThresholdType.VALUE ) )
            {
            ...
            if ( MetricFactory.hasThresholds( metricsConfig, ThresholdType.PROBABILITY_CLASSIFIER ) )
            {
                Set<MetricConstants> metrics = MetricFactory.getDichotomousMetrics();
                returnMe.addAll( metrics );
            }
        }

        return MetricFactory.removeMetricsDisallowedByOtherConfig( projectConfig, returnMe );
    }
