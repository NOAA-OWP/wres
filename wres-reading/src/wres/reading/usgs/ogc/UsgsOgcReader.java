 ...
 *
 * <p>Time-series are chunked by feature and year ranges. The underlying format reader is a {@link UsgsOgcResponseReader}.
...
public class UsgsOgcReader implements TimeSeriesReader
{
    /** Logger. */
        ...
        Function<WebClient.ClientResponse, String> errorUnpacker = response ->
        {
            if ( response.getStatusCode() == 429 )
            {
                ...
                return "Encountered an HTTP 429 Error, which happens when you request too much data from the "
                       + USGS + " within a short time period." + extra;
            }
            ...
            try
            {
                     ...
                     && ( remainingInt / ( double ) limitInt ) <= 0.1 )
                {
                    LOGGER.warn( "Requests to the {} are rate limited. The rate limit is {} and there are only "
                                 ...
                                 + "https://api.waterdata.usgs.gov/docs/ogcapi/", USGS, limitInt, remainingInt );
                }
            }
    ...
    /**
     * Attempts to retrieve the location metadata from a local cache, else from USGS. Attempts to populate the time
     * zone information for a given feature. Any daylight saving adjustment must be accounted for at the lowest level
     * of reading as Daylight Saving Time is, by definition, datetime dependent and, hence, requires the datetime for a
     * specific event.
     *
    ...
    private LocationMetadata getLocationMetadata( String featureId, DataSource dataSource )
    {
                                                                                        ...
                                                                                        HTTP_CLIENT_1_1 ) )
        {
            ...
            {
                // No feature metadata, but there is a universal time zone offset, which is good enough
                ...
                // No feature metadata, but this is actually required, i.e., daily values for which a feature-specific
                // time zone offset is needed, else the universal offset, which is not present (see above)
                ...
                // No feature metadata, but it isn't required, so use the empty placeholder
                else
                {
                    addMeToCache = new LocationMetadata( null, null, null, null );
                }
            }
            else
            {
                           ...
                           TimeSeriesReader geoJsonReader )
    {
        ...
        Objects.requireNonNull( geoJsonReader );
