...
import wres.reading.nwis.dv.response.MonitoringLocation;
...
/**
 * Reads time-series data from the National Water Information System (NWIS) Daily Values (DV) web service. This
 * service provides access to observed time-series whose event values always have a daily timescale. The service and
 * its API is described here:
 *
 * <p><a href="https://api.waterdata.usgs.gov/ogcapi/v0/collections/daily">USGS NWIS DV Web Service</a>
 *
 * <p>The above link was last accessed: 20251125T12:00Z.
 *
 * <p>Implementation notes:
 *
 * <p>Time-series are chunked by feature and year ranges. The underlying format reader is a
 * {@link NwisDvResponseReader}. The NWIS DV service supports paginated reads. All the pages generated by a single
 * request are read in a single thread. Parallel reads are instead achieved by chunking the data into prescribed time
 * intervals or chunks and a separate thread reads each chunk. Note that the NWIS DV service does not provide fully
 * qualified datetimes on a standard timeline. Rather, it supplies dates that implicitly represent a datetime of
 * midnight in local time. As such, feature-specific timing information must be obtained from a separate
 * monitoring_locations endpoint. This endpoint does not itself provide fully qualified time zone information, rather an
 * abbreviated time zone offset, together with a daylight savings indicator, which is ambiguous. This limits the shape
 * of reading, in practice. For example, it is not possible to read multiple features than span more than one time zone
 * in a single read as the timing information must be supplied as a sidecar to each read via the {@link DataSource}.
 *
 * @author James Brown
 */

public class NwisDvReader implements TimeSeriesReader
{
    /** Logger. */
    private static final Logger LOGGER = LoggerFactory.getLogger( NwisDvReader.class );

    /** The underlying format reader. */
    private static final NwisDvResponseReader GEOJSON_READER = NwisDvResponseReader.of();
    ...
    public static NwisDvReader of( EvaluationDeclaration declaration, SystemSettings systemSettings )
    {
        Objects.requireNonNull( declaration );

        return new NwisDvReader( declaration, systemSettings );
    }
    ...
    /**
     * This implementation is equivalent to calling {@link NwisDvResponseReader#read(DataSource, InputStream)}.
    ...
    private List<TimeSeriesTuple> readAllPages( DataSource dataSource )
    {
        ...
        for ( TimeSeriesTuple nextTuple : firstPage )
        {
                          ...
                          .hasNextPage() )
            {
                ...
                {
                    throw new ReadException( "While reading from the NWIS Daily Values service, failed to adjust a "
                                             + "base URI to add the format specification.", e );
                }
            }
    ...
    private DataSource addTimeZoneOffset( String featureId, DataSource dataSource )
    {
        if ( Objects.nonNull( dataSource.source()
                                        .timeZoneOffset() ) )
        {
            LOGGER.debug( "When reading data from the National Water Information System Daily Values service, "
                          + "discovered an declared 'time_zone_offset' of {}. This offset will be applied when reading "
                                                                                        ...
                                                                                        null ) )
        {
            ...
            if ( Objects.nonNull( zoneId ) )
            {
                ...
                // If no daylight saving, use the raw offset, else the full time zone
                if ( !"Y".equalsIgnoreCase( feature.getProperties()
                ...
                {
                    // Need to work with what we've got, so use a mapping to acquire the proper time zone
     ...
     * <a href="https://api.waterdata.usgs.gov/ogcapi/v0/collections/daily/">https://api.waterdata.usgs.gov/ogcapi/v0/collections/daily/</a></p>
     *
                                ...
                                String featureName )
    {
        ...
        if ( !baseEnd.endsWith( ITEMS ) )
        {
            ...
            {
                throw new ReadException( "While reading from the NWIS Daily Values service, failed to adjust a base "
                                         + "URI to add the /items path element.", e );
            }
        }
    ...
    private NwisDvReader( EvaluationDeclaration declaration, SystemSettings systemSettings )
    {
