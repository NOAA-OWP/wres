...
public class DeclarationFactory
{
                                              ...
                                              boolean validate ) throws IOException
    {
        ...
        String declarationString = DeclarationFactory.getDeclarationString( yamlOrPath, fileSystem );
        ...
        // Validate against an old-style XML string
        DeclarationFactory.validateAgainstOldDeclarationString( declarationString );
     ...
     * {@link DeclarationInterpolator#interpolate(EvaluationDeclaration, boolean)}. Also, does not perform any high-level
     ...
     *
     * @see DeclarationInterpolator#interpolate(EvaluationDeclaration, boolean)
    ...
    public static EvaluationDeclaration from( String yaml ) throws IOException
    {
        ...
        // Validate against an old-style XML string
        DeclarationFactory.validateAgainstOldDeclarationString( yaml );
    ...
    /**
     * Returns a declaration string from either a declaration string or path, reading from the path
     * as needed.
     * @param yamlOrPath the string containing declaration or a path
     * @param fileSystem a file system to use when reading a path, optional
     * @return the declaration string
     * @throws IOException if a path could not be read
     */

    public static String getDeclarationString( String yamlOrPath,
                                               FileSystem fileSystem ) throws IOException
    {
        ...
        if ( DeclarationUtilities.isReadableFile( fileSystem, yamlOrPath ) )
        {
            ...
            LOGGER.info( "Discovered a readable path to a declaration string: {}", path );
        }
     ...
     * itself, else unrecognized. Performs a shallow analysis only and does not fully parse or validate the string
     * against its schema.
     *
    ...
    public static boolean isValidDeclarationString( String declarationOrPath,
                                                    FileSystem fileSystem )
    {
        ...
        try
        {
            ...
            if ( DeclarationUtilities.isReadableFile( fileSystem, declarationOrPath ) )
            {
                ...
                declarationOrPath = Files.readString( path );
                LOGGER.info( "Discovered a path to a declaration string: {}", path );
            }

            return DeclarationFactory.isDeclarationString( declarationOrPath );
        }
        catch ( IOException e )
        {
    ...
    /**
     * Validates against an old-style declaration string.
     * @param test the test string
     * @throws IOException if the format could not be detected
     */
    ...
    private static void validateAgainstOldDeclarationString( String test ) throws IOException
    {
        ...
        if ( !oldError.isEmpty() )
        {
            throw new DeclarationException( oldError.get( 0 )
                                                    .getEventMessage() );
        }
    }
