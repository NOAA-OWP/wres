...
public final class ScriptGenerator
{
            ...
            throws SQLException
    {
        ...
        script.addLine("    FROM (");
        script.addLine("        SELECT MIN(TS.initialization_date),");
        script.addLine("            MAX(TS.initialization_date)");
        ...
        {
            script.addLine("            AND TS.initialization_date >= '",
        ...
        script.addLine("            )");
        script.addLine("        ) AS F");
        script.addLine("    CROSS JOIN (");
        script.addLine("        SELECT MIN(O.observation_time), MAX(O.observation_time)");
        ...
        script.addLine("    ) AS O");
        script.addLine(")");
        script.addLine("SELECT MIN(TS.initialization_date)::text,");
        script.addLine( "    ( EXTRACT( epoch FROM ( MAX( TS.initialization_date ) " );
        script.addLine( "                           - MIN( TS.initialization_date )" );
        script.addLine( "                           - INTERVAL '",
                        projectDetails.getIssuePoolingWindowPeriod(),
                        " " ,
                        projectDetails.getIssuePoolingWindowUnit(),
                        "' ) )");
        script.addLine( "     /");
        script.addLine( "     EXTRACT( epoch FROM ( INTERVAL '",
                        projectDetails.getIssuePoolingWindowFrequency(),
                        " ",
                        projectDetails.getIssuePoolingWindowUnit(),
                        "' ) )" );
        script.addLine( "    )::int + 1 AS window_count" );
        script.addLine("FROM wres.TimeSeries TS");
