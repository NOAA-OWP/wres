...
if [ $# -lt 1 ]
then
        echo "Usage: $0 -c config_file -z wresZipFile -l LOGFILE"
...
if [ -z "$CONFIGFILE" ]
then
	echo "Please provide -c config_file"
	exit 2
fi
if [ -z "$LOGFILE" ]
then
	LOGFILE=defaultLog.txt
fi
...
then
	# TODO
	echo "Search the latest graphics zip" 
	if [ -f build_recent_wres-vis.html ]
	then
		rm -v build_recent_wres-vis.html 2>&1 | /bin/tee -a $LOGFILE
	fi
	...
	if [ -f build_recent_wres-vis.html ]
	then
		LATEST_ZIPFILE=`$PYTHON_SCRIPT_DIR/searchInstallBuild_wres-vis.py build_recent_wres-vis.html | grep Data | gawk '{print ($2)}' | tr -d "')"`
		echo "The latest wres-vis zip file = $LATEST_ZIPFILE" 2>&1 | /bin/tee -a $LOGFILE
	fi
	if [ -n "$LATEST_ZIPFILE" ]
	then
		if [ -s /wres_share/releases/archive/graphics/$LATEST_ZIPFILE ]
		then
			echo -n "File " 2>&1 | /bin/tee -a $LOGFILE
			ls /wres_share/releases/archive/graphics/$LATEST_ZIPFILE 2>&1 | /bin/tee -a $LOGFILE
			echo " already downloaded"  2>&1 | /bin/tee -a $LOGFILE
			exit
		elif [ ! -f /wres_share/releases/archive/graphics/$LATEST_ZIPFILE ]
		then
			echo "Please wait .... currently is downloading the $LATEST_ZIPFILE file" 2>&1 | /bin/tee -a $LOGFILE
            $CURL --config $CONFIGFILE --silent --remote-name https://vlab.***REMOVED***/jenkins/job/Verify_OWP_WRES/ws/wres-vis/build/distributions/$LATEST_ZIPFILE
			if [ -s $LATEST_ZIPFILE ] # file exist and  size greater than 0
            then
				latest_noZip=`ls $LATEST_ZIPFILE | sed -e s/\.zip//`
	...
	then
			echo "Can't download $LATEST_ZIPFILE" 2>&1 | /bin/tee -a $LOGFILE 
	fi
fi
