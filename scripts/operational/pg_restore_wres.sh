...
do
    case $opt in
        ...
        p)
            ...
            ;;
        \?)
            echo "Usage: $0 -f dump_file_prefix -h database_host -d database_name -U database_username [ -k pg_restore_command -p port ]"
            exit 2
            ;;
    esac
done
...
list_objects_command="$pg_restore_command -e -p ${port} -l ${dump_file} -f ${list_of_non_schema_objects_file}"
restore_pre_data_only_command="$pg_restore_command -e -p ${port} -j $j --no-owner -h ${database_host} -d ${database_name} -U ${database_username} --section=pre-data"
restore_data_only_table_command="$pg_restore_command -e -p ${port} -j $j --no-owner -h ${database_host} -d ${database_name} -U ${database_username} --data-only --strict-names "
restore_post_data_only_command="$pg_restore_command -e -p ${port} -j $j --no-owner -h ${database_host} -d ${database_name} -U ${database_username} --section=post-data"
...
&& $restore_data_only_table_command -t measurementunit -t feature -t ensemble -t source $dump_file \
&& $restore_data_only_table_command -t measurementunit_measurementunit_id_seq -t ensemble_ensemble_id_seq -t source_source_id_seq $dump_file \
