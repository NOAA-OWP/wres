# Gets an existing project dataset as if it were forecasts.
...
select F.name as location,
    ...
    E.ensemble_name as ensemble_name,
...
ORDER BY TS.source_id, TS.ensemble_id
EOF

psql -h ${db_hostname} -p ${db_port} -d ${db_name} -U ${db_username} -c "\copy ( ${forecast_query} ) TO project_${project_id}_${member}_forecasts.csv CSV HEADER"
